...
| Point Color (pick a color from the photo, adjust it precisely) | LRC 13+ | REJECTED (2026-07-13 color-model decision, DESIGN.md): targeted color is the SECONDARY layer's job and the ColorKey rig is strictly more capable; invest in making ColorKey creation as fluid as Point Color's gesture instead. |
...
### Category E — thesis-completing additions (philosophy review 2026-07-13, none exist in the references)
| Feature | Notes |
|---|---|
| Sidecar visual diff ("code review for looks") | docs/brief-bank/git-native-completion.md §1 — param-language + visual diff of two look versions; the AI-loop review moment. Highest-priority unstarted feature per the review. |
| Sidecar spec document | §2 — the AI's UI is the schema. |
| Reproducibility stamp | §3 — exports record engine+sidecar provenance. |
